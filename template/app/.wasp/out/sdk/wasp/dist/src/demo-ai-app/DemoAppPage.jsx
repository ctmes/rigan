import { createTask, deleteTask, generateGptResponse, getAllTasksByUser, updateTask, useQuery, } from "wasp/client/operations";
...
function NewTaskForm({ handleCreateTask, }) {
        ...
        try {
            ...
            if (err.statusCode === 402) {
                toast({
                    ...
                    action: (<ToastAction altText="Go to pricing page to buy credits/subscription" asChild>
    ...
    return (<div className="flex flex-col justify-center gap-10">
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between gap-3">
          <Input type="text" id="description" className="flex-1" placeholder="Enter task description" value={description} onChange={(e) => setDescription(e.currentTarget.value)} onKeyDown={(e) => {
            if (e.key === "Enter") {
                handleSubmit();
            }
        }}/>
          <Button type="button" onClick={handleSubmit} disabled={!description} variant="default" size="default">
            Add Task
          </Button>
        </div>
      </div>
      ...
      <div className="col-span-full space-y-10">
        ...
        {tasks && tasks.length > 0 ? (<div className="space-y-4">
            {tasks.map((task) => (<Todo key={task.id} id={task.id} isDone={task.isDone} description={task.description} time={task.time}/>))}
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between gap-3">
                <Label htmlFor="time" className="text-muted-foreground text-nowrap text-sm font-semibold">
                  How many hours will you work today?
                </Label>
                <Input type="number" id="time" step={0.5} min={1} max={24} className="min-w-28 text-center" value={todaysHours} onChange={(e) => setTodaysHours(+e.currentTarget.value)}/>
              </div>
            </div>
          </div>) : (<div className="text-muted-foreground text-center">
            Add tasks to begin
          </div>)}
      </div>

      <Button type="button" disabled={isPlanGenerating || tasks?.length === 0} onClick={() => handleGeneratePlan()} variant="default" size="default" className="w-full" data-testid="generate-schedule-button">
        {isPlanGenerating ? (<>
      ...
      {!!response && (<div className="flex flex-col">
...
function Todo({ id, isDone, description, time }) {
      ...
      <div className="flex w-full items-center justify-between">
        <div className="flex w-full items-center justify-between gap-5">
          <div className="flex items-center gap-3">
            <Checkbox checked={isDone} onCheckedChange={handleCheckboxChange} className="data-[state=checked]:bg-primary data-[state=checked]:border-primary"/>
          ...
          <div className="flex items-center gap-2">
            <Input id="time" type="number" min={0.5} step={0.5} className={cn("w-18 h-8 text-center text-xs", {
            "pointer-events-none opacity-50": isDone,
        })} value={time} onChange={handleTimeChange}/>
            <span className={cn("text-muted-foreground text-xs italic", {
        ...
        <div className="w-15 flex items-center justify-end">
          <Button variant="ghost" size="sm" onClick={handleDeleteClick} title="Remove task" className="text-destructive hover:text-destructive/80 h-auto p-1">
            <Trash2 size="20"/>
          </Button>
        </div>
      </div>
...
function Schedule({ schedule }) {
    return (<div className="flex flex-col gap-6 py-6" data-testid="schedule">
...
function TaskCard({ task, taskItems, }) {
    const taskPriorityToColorMap = {
    ...
    return (<Card className={cn("border-2", taskPriorityToColorMap[task.priority])}>
      ...
      <CardContent className="pt-0">
        {!!taskItems ? (<ul className="space-y-2">
          ...
          </ul>) : (<div className="text-muted-foreground text-center">
            OpenAI didn't return any Task Items. Try again.
          </div>)}
      </CardContent>
    </Card>);
}
function TaskCardItem({ description, time }) {
    ...
    return (<li className="flex items-center justify-between gap-4 rounded-md p-2">
      <div className="flex flex-1 items-center gap-3">
        <Checkbox checked={isDone} onCheckedChange={handleCheckedChange} className="data-[state=checked]:bg-primary data-[state=checked]:border-primary"/>
        <span className={cn("text-sm leading-tight", {
